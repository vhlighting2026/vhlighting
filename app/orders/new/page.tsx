...
export default function Page() {
 ...
 async function save(printAfter=false){setMessage("");if(!items.length){setMessage("Hãy thêm ít nhất một sản phẩm.");return}setBusy(true);const orderNo="VH-"+Date.now();const payload={order_no:orderNo,customer_id:customer?.id||null,customer_name:customer?.name||"Khách lẻ",customer_phone:customer?.phone||null,customer_address:customer?.address||null,subtotal,discount,shipping_fee:shippingFee,total,status:"Mới",note:note.trim()||null,payment_method:"Chuyển khoản",transfer_mode:"company_qr",transfer_content:""};const{data:order,error}=await supabase.from("orders").insert(payload).select().single();if(error){setMessage(error.message);setBusy(false);return}const{error:itemError}=await supabase.from("order_items").insert(items.map(x=>({...x,order_id:order.id})));if(itemError){await supabase.from("orders").delete().eq("id",order.id);setMessage(itemError.message);setBusy(false);return}router.push(`/orders/${order.id}${printAfter?"?print=1":""}`)}
