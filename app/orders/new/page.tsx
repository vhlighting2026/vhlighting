...
import type { Customer, Product, OrderItem, CompanySettings } from "@/lib/types";
...
export default function Page() {
  ...
  const [transferMode, setTransferMode] = useState<"order"|"customer_input">("order");
  const [transferContent, setTransferContent] = useState("THANH TOAN HD");
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("company_settings").select("transfer_content_mode,transfer_content_default").eq("id",1).maybeSingle(),
    ]).then(([a, b, c]) => { setCustomers(a.data || []); setProducts(b.data || []); if(c.data?.transfer_content_mode)setTransferMode(c.data.transfer_content_mode); if(c.data?.transfer_content_default)setTransferContent(c.data.transfer_content_default); });
  }, []);
  ...
  function addProduct(product: Product) {
    setItems(current => {
      ...
      return [...current, { product_id: product.id, sku: product.sku, product_name: product.name, unit: product.unit, light_attribute: product.light_attribute||null, quantity: 1, unit_price: Number(product.price), line_total: Number(product.price) }];
    });
    setProductQuery("");
  ...
  async function save() {
    ...
    const { data: order, error } = await supabase.from("orders").insert({ order_no: orderNo, customer_id: customer?.id || null, customer_name: customer?.name || "Khách lẻ", customer_phone: customer?.phone || null, customer_address: customer?.address || null, subtotal, discount, shipping_fee: shippingFee, total, status: "Mới", note, transfer_mode: transferMode, transfer_content: transferMode==="order" ? (transferContent.trim() || null) : null }).select().single();
    ...
    <div className="panel order-editor">
      <div className="order-top-grid">
        ...
        <div className="product-picker"><label>Nhập mã hoặc tên sản phẩm</label><div className="search-box large"><Search size={18}/><input value={productQuery} placeholder="Ví dụ: VH-AT hoặc âm trần…" onFocus={() => setShowResults(true)} onChange={e => { setProductQuery(e.target.value); setShowResults(true); }}/></div>
          {showResults && productQuery && <div className="product-results">{searchResults.map(x => <button key={x.id} type="button" onClick={() => addProduct(x)}><div><span className="sku-pill">{x.sku}</span><b>{x.name}</b><small>{x.light_attribute||'Không thuộc tính'} · {x.unit} · Tồn {x.stock}</small></div><strong>{Number(x.price).toLocaleString("vi-VN")} ₫</strong><Plus size={17}/></button>)}{!searchResults.length && <div className="empty-state">Không có sản phẩm phù hợp</div>}</div>}
        </div>
      </div>
      ...
      <div className="table-wrap order-table"><table><thead><tr><th>STT</th><th>Mã</th><th>Tên sản phẩm</th><th>Thuộc tính</th><th>ĐVT</th><th>SL</th><th>Đơn giá</th><th>Thành tiền</th><th></th></tr></thead><tbody>
        {items.map((x, i) => <tr key={x.product_id || i}><td>{i + 1}</td><td><span className="sku-pill">{x.sku}</span></td><td><b>{x.product_name}</b></td><td>{x.light_attribute||'—'}</td><td>{x.unit}</td><td><input className="mini" type="number" min="1" value={x.quantity} onChange={e => patch(i, "quantity", Number(e.target.value))}/></td><td><input className="money" type="number" min="0" value={x.unit_price} onChange={e => patch(i, "unit_price", Number(e.target.value))}/></td><td className="number-cell"><b>{x.line_total.toLocaleString("vi-VN")} ₫</b></td><td><button className="icon-danger" onClick={() => setItems(v => v.filter((_, n) => n !== i))}><Trash2 size={16}/></button></td></tr>)}
        {!items.length && <tr><td colSpan={9} className="empty-state">Nhập mã sản phẩm phía trên để thêm vào đơn.</td></tr>}
      </tbody></table></div>

      <div className="checkout"><div><label>Ghi chú</label><textarea placeholder="Ghi chú giao hàng, bảo hành hoặc thanh toán…" value={note} onChange={e => setNote(e.target.value)}/><label style={{marginTop:12}}>Quy tắc nội dung chuyển khoản</label><select value={transferMode} onChange={e=>setTransferMode(e.target.value as "order"|"customer_input")}><option value="order">Theo đơn hàng</option><option value="customer_input">Khách hàng tự nhập</option></select>{transferMode==="order"&&<><label style={{marginTop:12}}>Nội dung chuyển khoản</label><input value={transferContent} onChange={e=>setTransferContent(e.target.value)} placeholder="VD: THANH TOAN HD + SDT KH"/></>}</div><div className="totals">
